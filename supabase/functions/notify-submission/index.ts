...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const subject = isPurchase
      ? `💰 PURCHASE: ${zip || "unknown"} — ${verdict_label || "N/A"}${leadEmail ? ` — ${leadEmail}` : ''} — ? `💰 PURCHASE: ${zip || "unknown"} — ${verdict_label || "N/A"}${leadEmail ? ` — ${leadEmail}` : ''} — $1.99`
      : leadEmail
    ...
    const purchaseBanner = isPurchase
      ? `<div style="background:#d4edda;border:1px solid #28a745;border-radius:8px;padding:16px;margin-bottom:16px;text-align:center">
           ...
           <h3 style="margin:8px 0 4px;color:#155724;font-size:18px"><h3 style="margin:8px 0 4px;color:#155724;font-size:18px">New Purchase — $1.99</h3></h3>
