...
function buildRenewalHtml(data: any) {
  const { email, city, state, zip, bedrooms, fairness_score, verdict_label } = data;
  ...
  const scoreLine = fairness_score != null
    ? `<p style="font-size:18px;font-weight:700;color:#2d6a4f;margin:16px 0 4px;">Your Fairness Score: ${fairness_score}/100 — ${verdict_label || "See report"}</p>`
    : "";
  ...
  return `
    <div style="font-family: 'DM Sans', Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px;">
      ...
      <h1 style="font-size: 20px; color: #1a1a1a; margin: 24px 0 12px;">Your rent analysis is ready</h1>
      ${scoreLine}
      ...
      <a href="${reportUrl}" style="display:inline-block;padding:14px 24px;background:#2d6a4f;color:#fff;border-radius:8px;text-decoration:none;font-size:15px;font-weight:600;">
...
function buildWsipHtml(data: any) {
  ...
  return `
    <div style="font-family: 'DM Sans', Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px;">
      ...
      <h1 style="font-size: 20px; color: #1a1a1a; margin: 24px 0 12px;">Your market report is ready</h1>
      <p style="font-size: 15px; color: #555; line-height: 1.7; margin: 16px 0;">
      ...
      <p style="font-size: 14px; color: #777; margin-top: 20px; line-height: 1.6;">
        Already have a lease? <a href="${renewalUrl}" style="color:#2d6a4f;text-decoration:underline;">Check if your next rent increase is fair →</a>
      </p>
      <p style="font-size: 15px; color: #555; margin-top: 28px;">— RenewalReply</p>
      <hr style="border: none; border-top: 1px solid #eee; margin: 32px 0 16px;" />
      <p style="font-size: 11px; color: #999; text-align: center;">
