...
ALTER TABLE public.leads
  ADD COLUMN IF NOT EXISTS original_capture_source text,
  ADD COLUMN IF NOT EXISTS first_captured_at timestamptz;

UPDATE public.leads
SET
  original_capture_source = capture_source,
  first_captured_at = created_at
WHERE original_capture_source IS NULL;
...
CREATE OR REPLACE FUNCTION public.upsert_lead(
  ...
  p_tool_type text DEFAULT 'renewal'
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  INSERT INTO public.leads (
    email, analysis_id, capture_source, original_capture_source, first_captured_at,
    ...
    fairness_score, comp_median_rent, hud_fmr_value, tool_type
  ) VALUES (
    p_email, p_analysis_id, p_capture_source, p_capture_source, now(),
    ...
    p_fairness_score, p_comp_median_rent, p_hud_fmr_value, p_tool_type
  )
  ON CONFLICT (email) DO UPDATE SET
    ...
    tool_type = COALESCE(EXCLUDED.tool_type, leads.tool_type);
END;
$$;
