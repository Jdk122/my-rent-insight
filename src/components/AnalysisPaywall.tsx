...
export default function AnalysisPaywall({
  ...
  // ── Headline (different angle than verdict — escalates, doesn't repeat) ──
  ...
  // ── Agitation (above + at-market only, shown ABOVE cards) ──
  ...
  return (
    <motion.div
      ...
      className="max-w-[480px] mx-auto mt-8 pt-6 px-4 overflow-hidden"
    >
      {/* ── Badge ── */}
      ...
      {/* ── Agitation (ABOVE cards — best copy on the page, put it where people see it) ── */}
      {agitation && (
        ...
        <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 -mx-2 px-2 sm:grid sm:grid-cols-3 sm:overflow-visible sm:mx-0 sm:px-0 sm:pb-0" role="list">
          {[card1, card2, { label: 'YOUR REPLY', value: null, sub: 'ready to copy and send', color: '' }].map((card, i) => (
            ...
            >
              ...
              {/* Blur overlay — SIBLING of content div, absolute positioned, z-10 renders ON TOP */}
              <div
      ...
      {/* ── PRIMARY CTA BUTTON — big, branded, works for 100% of users ── */}
      ...
      {/* ── Express Checkout (secondary — Apple Pay / Google Pay / Stripe Link) ── */}
      ...
      {/* ── Value stack (below the fold on mobile — rewards scrollers) ── */}
