...
function buildFallbackLetter(props: {
  ...
  paragraphs.push(`---\nGenerated by RenewalReply.com using publicly available market data including HUD Fair Market Rents, Zillow ZORI, and comparable listings. This letter is for informational purposes only and does not constitute legal advice.`);
    ...
    <div>
      ...
      >
        ...
        <div className="space-y-4">
          {displayLetter.split('\n\n').map((para, i) => {
            // Detect the "Generated by" footer paragraph
            const isGeneratedFooter = para.startsWith('---\nGenerated by RenewalReply.com') || para.startsWith('Generated by RenewalReply.com');
            if (isGeneratedFooter) {
              const text = para.replace(/^---\n/, '');
              return (
                <div key={i} className="text-[11px] text-muted-foreground/50 mt-4 pt-3 border-t border-border/30 leading-relaxed">
                  {text}
                </div>
              );
            }
            return <p key={i} className="text-sm text-foreground/80 leading-[1.7] whitespace-pre-line">{para}</p>;
          })}
        </div>
      </div>
