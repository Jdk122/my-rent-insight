...
const TickerItem = ({ item }: { item: typeof MARKET_DATA[number] }) => {
  ...
  return (
    <span className="inline-flex items-center gap-[6px] shrink-0">
      ...
      <span className="text-muted-foreground text-[11px] sm:text-[12px]">{item.bed}</span>
      <span className="text-foreground font-medium text-[12px] sm:text-[13px]">
...
const MarketTicker = () => {
  ...
  const items = MARKET_DATA.map((item, i) => (
    <span key={i} className="inline-flex items-center shrink-0">
      ...
      <span className="text-border/50 text-[10px] mx-2">|</span>
    </span>
  ));

  return (
    <div
      className="w-full border-b border-border bg-muted/30 py-2 overflow-hidden relative"
    ...
    >
      ...
      <div
        ref={contentRef}
        className="flex items-center motion-safe:animate-[ticker-scroll_var(--ticker-duration)_linear_infinite]"
        style={{
