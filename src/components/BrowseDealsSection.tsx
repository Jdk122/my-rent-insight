...
const NEIGHBORHOODS: { label: string; slug: string; sub: string }[] = [
  { label: 'East Village', slug: 'east-village', sub: 'Manhattan, NY' },
  { label: 'Chelsea', slug: 'chelsea-nyc', sub: 'Manhattan, NY' },
  { label: 'Brickell', slug: 'brickell', sub: 'Miami, FL' },
  { label: 'Lincoln Park', slug: 'lincoln-park', sub: 'Chicago, IL' },
  { label: 'Mission District', slug: 'mission-district', sub: 'San Francisco, CA' },
  { label: 'Wicker Park', slug: 'wicker-park', sub: 'Chicago, IL' },
];
      ...
      {NEIGHBORHOODS.map(n => (
        <Link key={n.slug} to={`/deals/${n.slug}`} className={cardClass}>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[13px] text-muted-foreground leading-relaxed mt-0.5">{n.sub}</p>
          </div>
